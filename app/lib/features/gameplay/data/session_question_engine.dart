...
class SessionQuestionEngine {
  ...
  GenerationResult generateQueue({
    ...
    return GenerationResult(
      queue: _ensureFirstQuestionIsEngaging(queue),
      packIds: packIds,
  ...
  /// Ensures the first question in [queue] is from an engaging, easy-to-answer
  /// category so the first round creates an immediate hook.
  ///
  /// Scans the first 40% of the queue for a question from [_engagingCategories].
  /// If one is found at index > 0, it is swapped to position 0.
  /// If none exists in that window, the queue is returned unchanged.
  List<SessionQuestion> _ensureFirstQuestionIsEngaging(
      List<SessionQuestion> queue) {
    if (queue.isEmpty) return queue;

    const engagingCategories = {
      CategoryKeys.funnyChaos,
      CategoryKeys.majlisGcc,
      CategoryKeys.friends,
    };

    final limit = (queue.length * 0.4).ceil().clamp(1, queue.length);
    final idx = queue.indexWhere(
      (q) => engagingCategories.contains(q.packId),
    );

    if (idx > 0 && idx < limit) {
      final swapped = List<SessionQuestion>.from(queue);
      final first = swapped.removeAt(idx);
      swapped.insert(0, first);
      return swapped;
    }
    return queue;
  }
