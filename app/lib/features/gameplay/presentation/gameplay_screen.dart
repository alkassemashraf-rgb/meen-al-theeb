...
class _GameplayScreenState extends ConsumerState<GameplayScreen> {
  ...
  // Current round (updated each build for first-round detection)
  GameRound? _currentRound;

  // Reaction push overlay: shown ~1.5s after reveal on first round only
  bool _showReactionPush = false;
  bool _reactionPushScheduled = false;
  ...
  /// True when the current round is the first round of the session.
  bool get _isFirstRound => _currentRound?.roundNumber == 1;
  ...
  void _startVotingSequence(String roundId) {
    ...
    // Step 1: hook text — longer on first round to land the message
    _introTimer = Timer(Duration(milliseconds: _isFirstRound ? 1500 : 600), () {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return roundAsync.when(
      data: (round) {
        ...
        // Update current round for _isFirstRound detection (no setState needed)
        _currentRound = round;
        ...
        if (isReveal && !_revealScheduled) {
          ...
          _showReactionPush = false;
          _reactionPushScheduled = false;
        }
        ...
        // First round only: show reaction push nudge ~1.5s after reveal
        if (isReveal && _revealReady && _isFirstRound && !_reactionPushScheduled) {
          _reactionPushScheduled = true;
          Future.delayed(const Duration(milliseconds: 1500), () {
            if (mounted) {
              setState(() => _showReactionPush = true);
              Future.delayed(const Duration(milliseconds: 2500), () {
                if (mounted) setState(() => _showReactionPush = false);
              });
            }
          });
        }
        ...
        return PageContainer(
          ...
          child: Stack(
            children: [
              ...
              // First round only: reaction push nudge above the emoji bar
              if (showReveal && _showReactionPush)
                Positioned(
                  left: 0,
                  right: 0,
                  bottom: 80,
                  child: IgnorePointer(
                    child: AnimatedOpacity(
                      opacity: _showReactionPush ? 1.0 : 0.0,
  ...
  void _showResultCard(GameRound round, AsyncValue<Room?> roomAsync) {
    ...
    showResultCardSheet(context, payload, roomCode: room.joinCode);
  }
...
// ---------------------------------------------------------------------------
// Winner Glow — pulses on first round
// ---------------------------------------------------------------------------

/// Wraps a winner avatar in a circular glow that pulses on the first round.
///
/// When [pulse] is true, an [AnimationController] runs in ping-pong mode,
/// animating the BoxShadow spread between 4 and 14 to create a heartbeat
/// effect. When [pulse] is false, a static glow is rendered instead.
class _WinnerGlow extends StatefulWidget {
  final Widget child;
  final bool pulse;
  const _WinnerGlow({required this.child, required this.pulse});
...
class _WinnerGlowState extends State<_WinnerGlow>
  ...
  void initState() {
    ...
    _spread = Tween<double>(begin: 4, end: 14).animate(
